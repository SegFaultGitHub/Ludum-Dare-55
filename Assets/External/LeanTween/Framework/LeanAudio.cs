    ...
    public class LeanAudioStream {
        ...
        public void OnAudioRead(float[] data) {
            ...
            while (count < data.Length) {
                data[count] = this.audioArr[this.position];
                this.position++;
                count++;
            }
        }
        ...
        private static int createAudioWave(AnimationCurve volume, AnimationCurve frequency, LeanAudioOptions options) {
            ...
            for (int i = 0; i < PROCESSING_ITERATIONS_MAX; i++) {
                ...
                if (listLength >= PROCESSING_ITERATIONS_MAX - 1) {
                    Debug.LogError(
                        $"LeanAudio has reached it's processing cap. To avoid this error increase the number of iterations ex: LeanAudio.PROCESSING_ITERATIONS_MAX = {PROCESSING_ITERATIONS_MAX * 2}"
                    );
                    break;
                }
 ...
 */
     ...
     *     AnimationCurve frequencyCurve = new AnimationCurve( new Keyframe(0f, 0.003f, 0f, 0f), new Keyframe(1f, 0.003f, 0f, 0f));
     *     <br>
     *         AudioClip audioClip = LeanAudio.createAudio(volumeCurve, frequencyCurve, LeanAudio.options().setVibrato( new Vector3[]{ new Vector3(0.32f,0.3f,0f)}
     *         ).setFrequency(12100) );<br>
     */
